...
fig.suptitle("Iris Dataset — Exploratory Data Analysis", fontsize=16, fontweight='bold', y=1.01)

palette = {"setosa": "#4CAF50", "versicolor": "#2196F3", "virginica": "#F44336"}

# 2a. Pairplot-style scatter: Sepal
ax = axes[0, 0]
...
# Feature Importance (Random Forest) or Coefficient (Logistic Regression)
if best_name == "Random Forest":
    importances = best["model"].feature_importances_
    feat_names  = ['Sepal\nLen', 'Sepal\nWid', 'Petal\nLen', 'Petal\nWid']
    colors_bar  = ['#4CAF50' if i == np.argmax(importances) else '#90CAF9' for i in range(4)]
    axes[1].bar(feat_names, importances, color=colors_bar, edgecolor='white', alpha=0.9)
    ...
    feat_names = ['Sepal\nLen', 'Sepal\nWid', 'Petal\nLen', 'Petal\nWid']
    axes[1].bar(feat_names, coef, color='#2196F3', alpha=0.85)
    axes[1].set_title("Mean |Coefficient| per Feature", fontsize=13, fontweight='bold')
    axes[1].set_ylabel("Mean |Coefficient|", fontsize=11)
    axes[1].grid(axis='y', alpha=0.3)
else:
    # Generic: just show accuracy per class
    per_class_acc = cm.diagonal() / cm.sum(axis=1)
    axes[1].bar(iris.target_names, per_class_acc,
                color=[palette[n] for n in iris.target_names], alpha=0.85)
